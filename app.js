...
const API_KEY = 'eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIwZDJhZjg2Yzc2NWUwYzUzOTA3ZDY5OTM2MzI3ZDgxZSIsInN1YiI6IjVmY2MzNTkxYTU3NDNkMDAzZjA2N2RmYSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.MHRxksSqi4yRxHehyxGhu7AJGeRjVD3rBr38LfbAYmw';
  ...
  return `
    <div class="card m-2" style="width: 20rem;">
      ...
      <div class="card-body d-flex flex-column justify-content-between">
        ...
        <p class="card-text">Pais(es) de origem   :   ${data.origin_country}</p>
        <p class="card-text">Data de lançamento: ${data.first_air_date}</p>
        <p class="card-text">Nota: ${data.vote_average}</p>
        <a href="#" class="btn btn-red btn-movie-card" data-movie-id="${data.id}">Mais Informações</a>
      </div>
    </div>
  `
};
...
const setModalContent = (data) => {
  ...
  document.querySelector('#modalTitle').innerHTML = data.name;
...
const getTrending = async () => {
  switchLoader(true);
  await fetch(`${API_ROOT_URL}trending/tv/week?${DEFAULT_PARAMS}`, {
    method: 'GET',
...
const getSingleMovie = async (movieId) => {
  switchLoader(true);
  await fetch(`${API_ROOT_URL}tv/${movieId}?${DEFAULT_PARAMS}`, {
    method: 'GET',
    headers: DEFAULT_HEADERS
  })
    .then(async response => {
